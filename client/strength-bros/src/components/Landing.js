...
class Landing extends Component {
  ...
  render() {
    if (this.state.isJoin) {
      return (
        <AthleteView user_id={this.state.user_id} room_id={this.state.value} />
      );
    }
    ...
    return (
      <div className="App">
        ...
        <div className="gridChild container center-block">
          ...
          <div className="error">
            {this.state.failedCodeIn ? "Code cannot be blank" : ""}
          </div>
          <div className="error">
            {this.state.failedUserIn ? "Username cannot be blank" : ""}
          </div>
        </div>
        <div className="gridChild">
          <form onSubmit={this.onClickCreate}>
            <button className="btn btn-large btn btn-outline-dark">
              Create Game
            </button>
          </form>
        </div>
